...
router.post('/ppt-to-pdf', upload.single('pptFile'), (req, res) => {
    ...
    try {
        if (process.platform === 'win32') {
            const psCommand = `powershell -ExecutionPolicy Bypass -File "${path.join(__dirname, '../scripts/convert-ppt-to-pdf.ps1')}" -pptxPath "${inputFilePath}" -pdfPath "${outputFilePath}"`;
